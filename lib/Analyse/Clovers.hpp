...
public:
  ...
  Bool_t Fill(Event const & event, int const & index)
  {
    ...
    if (isClover[label])
    {
      ...
      std::vector<uchar>::iterator finder = std::find(std::begin(m_clover_labels), std::end(m_clover_labels), clover_label);
      ...
      if (isGe[label])
      {
        std::vector<uchar>::iterator finder = std::find(std::begin(m_clover_Ge_labels), std::end(m_clover_Ge_labels), clover_label);
...
private:
  ...
  std::vector<uchar> m_clover_labels; // List of labels of the clovers that fired in the event
