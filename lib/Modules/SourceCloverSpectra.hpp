 ...
 */
class SourceCloverSpectra
{
public:
  SourceCloverSpectra() = default;
  SourceCloverSpectra(int argc, char** argv) {this -> Initialize(argc, argv);}
  void Initialize();
...
private:
  ...
  static void runMTFasterReader(Hit & hit, Alignator & tree, SourceCloverSpectra & ce)
  {
...
inline void SourceCloverSpectra::fillHisto(std::string const & datapath, int nb_files)
{
...
inline void SourceCloverSpectra::writeHisto(std::string const & outName)
{
...
inline void SourceCloverSpectra::printParameters()
{
  print("");
  print("Usage of SourceCloverSpectra : /path/to/data outputFile [[parameters]]");
  print("");
  print("parameters :");
  print("");
...
inline void SourceCloverSpectra::Initialize(int argc, char** argv)
{
  if (argc<2) 
  {
...
inline void SourceCloverSpectra::Initialize()
{
...
inline void SourceCloverSpectra::runFasterReader(Hit &hit, Alignator &tree)
{
  // Clover only
  Event cloversEvent;
