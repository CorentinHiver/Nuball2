...
public:
  ...
  explicit EventBuilder(Int_t const & i)
  {
    m_buffer = new Buffer;
    m_buffer->resize(2);
    m_event = new Event;
  ...
  explicit EventBuilder(Event  * _event) : m_event (_event) {}
  explicit EventBuilder(Buffer * _event)                            : m_buffer(_event){}
  explicit EventBuilder(Buffer * _event, Int_t const & _timeWindow) : m_buffer(_event), m_time_window(_timeWindow){}
  ...
  void setRF_ref_time(Time const & _RF_ref_time) {RF_ref_time = _RF_ref_time;}
...
Bool_t EventBuilder::build(Hit const & hit, RF_Manager const & /*unused*/)
{
...
//   if (hit.time-)
// }
void EventBuilder::set_last_hit(Hit const & hit)
{
  m_last_hit = hit;
