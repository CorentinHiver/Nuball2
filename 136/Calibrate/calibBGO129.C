...
void calibBGO129()
{
  ...
  auto file(TFile::Open("~/faster_data/N-SI-129-source_histo/Th232_both_sides.root", "READ"));
  {
    ...
    for (auto it : spectra)
    {
      ...
      // Fit 2614
      auto const & peak_ADC = spectrum->GetBinCenter(find_max_peak(spectrum, 0.02));
...
// i = 0;
    for (auto it : spectra)
    {
      ...
      // print(ADC_1274, ADC_511);

      spectrum->GetXaxis()->SetRangeUser(ADC_511*0.85, ADC_511*1.15);
  ...
  std::map<std::name, TGraph*> graphs;
...
// i = 0;
  ...
  file->cd();
  all_BGOs->Write();
  for (auto & name : names) spectra_clean[name].write();
