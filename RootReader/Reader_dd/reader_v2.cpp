...
public:
  ...
  static void setMaxHits(long const & nb_max_hits) {g_nb_max_hits = nb_max_hits;}
...
void Analysator::analyse(Nuball2Tree & tree, Event & event)
{
  ...
  { // Iterate over the events of the file
    ...
    {// Iterate over the hits of the event
      ...
      if (!isDSSD[label] && -10<time_ns && time_ns<10)
      {
    ...
    if ((clean_indexes.size() == 2) && (one_close_prompt))
    {
      ...
      auto const & prompt_mult =  nb_gamma_in_prompts[closest_prompt];
      if (dT>50) continue;
