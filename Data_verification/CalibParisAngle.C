...
void eff(TH1F* histo)
{
  ...
  print(histo->GetName(), ":", integral, "over", histo->Integral(), "->", integral/histo->Integral(), "% efficiency");
}
...
void Co60_efficiency()
{
  ...
  auto gated_cleanGe = new TH1F("gated_cleanGe", "gated_cleanGe;[keV]", 2000, 0, 2000);
  ...
  for (int evt_i = 0; evt_i<tree->GetEntries(); ++evt_i)
  {
    ...
    for (size_t hit_i = 0; hit_i<clovers.clean.size(); ++hit_i) 
    {
      ...
      if (gate(gate_low, int(nrj_i), gate_high+1))
      {
        ...
        if (clovers.clean.size()==1) 
        {
          gated_Ge->Fill(-1);
          gated_cleanGe->Fill(-1);
        }
        ...
        for (size_t hit_j = 0; hit_j<clovers.all.size(); ++hit_j)
        {
          ...
          if (clover_j.nrj>0 && clover_j.index() != index_i) 
          {
            ...
            {
              gated_cleanGe->Fill(clover_j.nrj);
