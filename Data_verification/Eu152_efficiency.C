...
void Eu152_efficiency(int nb_files = -1, long max_cursor = 1.e+10)
{
  ...
  std::string path = Path::home().string()+"nuball2/N-SI-136-root_sources/152Eu_center_2_before_calib/";
...
{
  if (argc == 1) Eu152_efficiency();
  else if (argc == 2) Eu152_efficiency(std::stoi(argv[1]));
  else if (argc == 3) Eu152_efficiency(std::stoi(argv[1]), std::stod(argv[2]));
  return 1;
}

// g++ -g -o Eu152_efficiency Eu152_efficiency.C ` root-config --cflags` `root-config --glibs` -DDEBUG -lSpectrum -std=c++17 -Wall -Wextra
// g++ -O2 -o Eu152_efficiency Eu152_efficiency.C ` root-config --cflags` `root-config --glibs` -lSpectrum -std=c++17