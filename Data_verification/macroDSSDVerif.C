  ...
  MTObject::parallelise_function([&](){
    ...
    while(MTfiles.getNext(file))
    {
      ...
      for (size_t ring_i = 0; ring_i<DSSD::nb_rings+DSSD::nb_sectors; ++ring_i)
      {
        ...
        E_VS_Clover_each.emplace_back(unique_TH2F(new TH2F((name).c_str(), (name+";Clover [keV];Strip [keV]").c_str(), 6_k,0,6_MeV, 7.5_k,0,30_MeV)));
      }
      ...
      while(tree.readNext())
      {
        if (MTObject::getThreadsNb()==1) progressbar.showEvery(freq_hit_display);
        else if (tree.cursor()%freq_hit_display == 0) print (nicer_double (tree.cursor()));
        ...
        // E_vs_angle->Fill(dssd.angle, dssd.Ex_p);
