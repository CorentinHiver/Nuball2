      ...
      for ( ;(evt_i < tree->GetEntries() && evt_i < nb_hits_read); evt_i++)
      {
        ...
        for (size_t loop_i = 0; loop_i<delayed_clovers.GeClean.size(); ++loop_i)
        {
          ...
          for (auto const & index_j : last_delayed_clovers.GeClean)
          {
            ...
            if (abs(clover_i.time-clover_j.time)<50_ns) continue;
          ...
          for (size_t loop_i = 0; loop_i<delayed_clovers.GeClean.size(); ++loop_i)
          {
            ...
            if (prompt_mult > 0 && Ex_p > 0)
            {
              ...
              for (size_t loop_j = loop_i+1; loop_j<delayed_clovers.GeClean.size(); ++loop_j)
              {
                ...
                auto const & clover_j = delayed_clovers[index_j];
                if (abs(clover_i.time - clover_j.time) < 50_ns)
                {
        ...
        if (Cmult[2])
        {
          ...
          if (abs(Ge0.time - Ge1.time) < 50_ns)
          {
        ...
        if (delayed_paris_mult == 2)
        {
          ...
          if (prompt_mult > 0)
          {
            ...
            if (dssd_trigger)
            {
              ...
              for (auto const & id : prompt_paris.back.modules_id) prompt_paris_VS_sum_paris_M2_pP->Fill(sum, prompt_paris.back.modules[id].nrj);
              for (auto const & id : prompt_paris.front.modules_id) prompt_paris_VS_sum_paris_M2_pP->Fill(sum, prompt_paris.front.modules[id].nrj);
            }
          }
        }
