...
public :
  ...
  constexpr inline Alias readNextHit() noexcept
  {
    ...
    if (n != sizeof(m_header))   error("in FasterReaderV2::readNext("+m_filename+") : header reading " + gzlibError(n));
    ...
    if (n != m_header.buff_size) error("in FasterReaderV2::readNext("+m_filename+") : data dumping " + gzlibError(n));
    if (m_header.magic != FASTER_MAGIC) error("in FasterReaderV2::readNext("+m_filename+") : magic check failed !!"+std::to_string(m_header.magic)+ "!="+std::to_string(FASTER_MAGIC));
    return true;
	}
  ...
  void loadTimeshifts(std::string const & tsFile) {m_timeshifts.load(tsFile)       ;}
  void loadTimeshifts(Timeshifts  const & tsData) {m_timeshifts = tsData           ;}
  void loadTimeshifts(Timeshifts       && tsData) {m_timeshifts = std::move(tsData);}
  ...
  {
    // Replaces the 4 manual members with a compile-time tuple
    std::tuple<
        QDCSpectro_xn<1>, 
        QDCSpectro_xn<2>, 
        QDCSpectro_xn<3>, 
