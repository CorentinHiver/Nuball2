...
public:
  ...
  {
    Time const shift = this->stamp - newTimestamp;
...
void inline Event::reading(TTree * tree, std::string const & options)
{
  ...
  if (readOpt.test(readOpt.m)) tree -> SetBranchAddress("mult"   , & mult   );
  if (readOpt.test(readOpt.l)) tree -> SetBranchAddress("label"  , & labels );
  if (readOpt.test(readOpt.t)) tree -> SetBranchAddress("stamp"  , & stamp  );
  if (readOpt.test(readOpt.T)) tree -> SetBranchAddress("time"   , & times  );
  if (readOpt.test(readOpt.e)) tree -> SetBranchAddress("adc"    , & adcs   );
  if (readOpt.test(readOpt.E)) tree -> SetBranchAddress("nrj"    , & nrjs   );
  if (readOpt.test(readOpt.q)) tree -> SetBranchAddress("qdc2"   , & qdc2s  );
  if (readOpt.test(readOpt.Q)) tree -> SetBranchAddress("nrj2"   , & nrj2s  );
  if (readOpt.test(readOpt.p)) tree -> SetBranchAddress("pileup" , & pileups);
...
void inline Event::writing(TTree * tree, std::string const & options)
{
  ...
  if (writeOpt.test(writeOpt.t)) ColibBis::createBranch     (tree,  "stamp"  , &stamp  );
  if (writeOpt.test(writeOpt.T)) ColibBis::createBranchArray(tree,  "time"   , &times  , "mult");
  if (writeOpt.test(writeOpt.E)) ColibBis::createBranchArray(tree,  "nrj"    , &nrjs   , "mult");
  if (writeOpt.test(writeOpt.Q)) ColibBis::createBranchArray(tree,  "nrj2"   , &nrj2s  , "mult");
  if (writeOpt.test(writeOpt.e)) ColibBis::createBranchArray(tree,  "adc"    , &adcs   , "mult");
  if (writeOpt.test(writeOpt.q)) ColibBis::createBranchArray(tree,  "qdc2"   , &qdc2s  , "mult");
  if (writeOpt.test(writeOpt.l)) ColibBis::createBranchArray(tree,  "label"  , &labels , "mult");
  if (writeOpt.test(writeOpt.p)) ColibBis::createBranchArray(tree,  "pileup" , &pileups, "mult");
